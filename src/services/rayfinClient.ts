...
import type { SunchaserSchema } from '../../rayfin/data/schema';

import { getDemoClient, isDemoMode } from './demoClient';
...
let client: RayfinClient<SunchaserSchema> | null = null;
let localDev = false;

export function initRayfinClient(
  config: RayfinClientConfig
): RayfinClient<SunchaserSchema> {
  if (client) {
    throw new Error('Rayfin client is already initialized.');
  }
  client = new RayfinClient<SunchaserSchema>({
...
export function getRayfinClient(): RayfinClient<SunchaserSchema> {
  if (isDemoMode()) {
    return getDemoClient() as unknown as RayfinClient<SunchaserSchema>;
  }
  if (!client) {
    throw new Error(
